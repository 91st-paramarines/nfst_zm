...
for "_i" from 1 to _planeNumber do
{
  ...
  _spawnPos   = [_spawnPosX   + _offsetSpawnX  , _spawnPosY   + _offsetSpawnY  , _flightHeight];
  _despawnPos = [_despawnPosX + _offsetDespawnX, _despawnPosY + _offsetDespawnY, _flightSpeed];
  _bombingPos = [_targetX     + _offsetSpawnX  - _bombGlideX, _targetY     + _offsetSpawnY - _bombGlideY, _flightHeight];
  _bombingEnd = [_targetEndX  + _offsetSpawnX  - _bombGlideX, _targetEndY  + _offsetSpawnY - _bombGlideY, _flightHeight];
...

{
    ...
    [_plane, _targetPos, _fireNull, _planeSide, _fire, _activeWeapon] spawn {
      params ["_plane","_targetPos", "_fireNull", "_planeSide", "_fire", "_activeWeapon"];
      waituntil {
    		_fireProgress = _plane getvariable ["fireProgress",0];

    		//--- Fire!
    		if ((getposasl _plane) distance _targetPos < 1000 && _fireNull) then {
    			...
    			_fireNull = false;
    			terminate _fire;
    			_fire = [_plane,_activeWeapon,_target] spawn {
